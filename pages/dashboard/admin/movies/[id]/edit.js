...
const MovieEdit = ({ movieId }) => {
  ...
  // New state for controlled input
  const [newMovieTimingInput, setNewMovieTimingInput] = useState('')

  // State for selected showroom
  const [selectedShowroom, setSelectedShowroom] = useState(null)

  // Set selectedShowroom once showRoomOptions are available
  useEffect(() => {
  ...
  const onSubmitHandler = async e => {
    ...
    try {
      const response = await axios.put(`/api/movies/moviedetails`, {
        ...movieDetails,
        title,
