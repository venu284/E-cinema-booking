...
export const ShowroomDeleteForm = () => {
  ...
  const handleDelete = async e => {
    const {
      data: { message }
    } = await axios.delete(`/api/showrooms?showroomId=${deleteId?._id}`)
    if (message == 'Showroom Deleted') {
      await mutate('/api/showrooms/')
      setIsOpen(false)
