...
export const ShowroomEditForm = () => {
  ...
  const onSubmitHandler = async e => {
    ...
    try {
      const { data } = await axios.put(`/api/showrooms/`, {
        ...deleteId,
        user: user?._id,
        name: showroomName
      })
      ...
      setLoading({ type: 'edit', status: false })
      if (data?.message == 'Showroom Updated') {
